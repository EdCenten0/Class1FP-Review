...
-- The second number represents the sum of the ev   en numbers in that list.

countSumEvens :: [[Int]] -> [[Int]]
countSumEvens [] = []
countSumEvens (x:xs) = [length (filter even x), sum (filter even x)] : countSumEvens xs
...
allEven [] = True
allEven (x:xs) 
    | even x = True && allEven xs
    | otherwise = False && allEven xs
...
consecutiveSums 0 [] = False
consecutiveSums n (x:xs)
    | x + head xs == n = True
    | otherwise = False && consecutiveSums n xs
